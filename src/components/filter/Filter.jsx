...
function Filter({ products, setProducts, className, children }) {
  ...
  const authors = Array.from(authorsName).map((item, i) => ({
    id: i,
    author: item,
  }));
  ...
  const filter = () => {
    const filtered = products.filter((product) => {
      ...
      if (filterParams.authors.length > 0) {
        condition = filterParams.authors.find(
          (author) => author === product.author
        );
      }
      return condition && product.price >= min && product.price <= max;
    });
    ...
    <div className={styles.filter}>
      <DropMenu caption={children} className={className}>
        <div className={styles.menu}>
          <h3 className={styles.menu__title}>Price</h3>
          ...
          <h3 className={styles.menu__title}>Author</h3>
